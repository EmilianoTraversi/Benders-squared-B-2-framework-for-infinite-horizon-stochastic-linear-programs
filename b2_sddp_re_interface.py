"""Solve random instances of the rebalancing problem.

This module provides a command line interface to create and solve
random instances of the continuous rebalancing problem, using the B^2
...
def register_options(parser):
    ...
    parser.add_argument('--sat', '-s', action = 'store',
                        dest = 'saturation', type = float, default = 1.0,
                        help = 'Percentage of the total ammount of  bicycles' +
                        ' used in the basic scenario. Default 1.0.')
    ...
    parser.add_argument('--num_points', '-nc', action = 'store',
                        dest = 'num_points', type = int, default = 10,
...
def run_random_instances(algorithm, num_instances, num_points,
                         num_scenarios, saturation, delta,
                         detailed_output_stream):
    """Solve random rebalancing instances and collect statistics.
    ...
    saturation : float
        Percentage of the total ammount of bicycles used in the base
        scenario.  Must be between 0 and 1.
    ...
    print('** npoints : {:>6d}'.format(num_points),
          '  k : {:>6d}'.format(num_scenarios),
          '  sat : {:>5.34}'.format(saturation),
    ...
    for i in range(num_instances):
        instance = re.RandomRebalancing(num_points, num_scenarios,
                                        delta, saturation)
...
if (__name__ == "__main__"):
    ...
    run_random_instances(args.algorithm, args.num_instances,
                         args.num_points, args.num_scenarios,
                         args.saturation, args.delta,
                         outstream)
    outstream.flush()
    outstream.close()
